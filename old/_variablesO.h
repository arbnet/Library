...
namespace v {
	...
	struct isMatch<T0,T0> {static const bool value = true;};
	template <class oCLASS, class = void>
	struct HasClass : std::false_type {};
	template <class oCLASS>
	struct HasClass<oCLASS, decltype(void(&oCLASS::Class))> : std::true_type {};
	template <class oCLASS>
	constexpr bool isClass=HasClass<oCLASS>::value;
		...
		if(sz){
			if(this->sz){
				...
				if(this->tl){
					...
					adr.a1=this->str;v::shift(adr.a1,this->sz);
				}else{
				...
				if(sz){
					adr.a2=pnt;v::copy(adr,sz);*(BYTE*)adr.a1=0;
				}
				this->sz=sn;
...
namespace v {
	...
	template <class oCLASS>
	STRING type(oCLASS &obj){
		if constexpr (isClass<oCLASS>) return obj.Class();
	...
	LOGIC istype(STRING st1,STRING st2){return st1==st2?true:false;}
}
...
private:
	...
	template<typename dTYPE>
	void Init(dTYPE &obj){
		this->mtp=v::type(obj);this->pnt=(POINTER)&obj;
		std::cout << "I=" << this->mtp << std::endl;
	}
public:
	Link(){}
	template<typename dTYPE>
	Link(dTYPE &obj){this->Init(obj);}
	POINTER operator *() const{return this->pnt;}
	template<typename dTYPE>
	...
	STRING Type(){
		std::cout << "TTTT" << this->mtp << std::endl;
		return this->mtp;}
...
public:
	int a=12345;
	STRING Class(){return STRING("classA");}
};
...
public:
	...
	~ANY(){this->Clear();}
	Link& operator*(){return *(Link*)this->pnt;}
	template <typename dTYPE>
	explicit operator dTYPE(){
		if(this->vtp!="Link")return *static_cast<dTYPE*>(this->pnt);
		else{
			std::cout << "2pnt#"<< this->pnt << " val#"<< ((A)*(Link*)this->pnt).a << std::endl;
			//*static_cast<dTYPE*>
			return static_cast<dTYPE>(*(Link*)this->pnt);
		}
	}
	template <typename dTYPE>
	ANY& operator=(dTYPE val){
		...
		if(this->vtp!=ntp){this->Clear();
			if(v::idtype(ntp)){
				...
				this->vtp="Link";this->pnt=new Link(val);
				std::cout << "1#"<< &val<< " pnt#"<<  this->pnt<< "_"<< ((A)*(Link*)this->pnt).a <<std::endl;
			}
		}else *(dTYPE*)this->pnt=val;
		return *this;
	}
	ANY& operator=(CHARS(ltr)){
		*this=STRING(ltr);return *this;
	}
	ANY& operator=(ANY obj){
		switch(v::idtype(obj.Type())){
			...
			default:	*this=(Link)obj;
		}
		return *this;
	}
	STRING Type(){
		return STRING("UUU");
		//if(this->vtp!="Link")return this->vtp;
		//else return (*(Link*)this->pnt).Type();
	}
	LETTER TT(){
		//STRING *str=NULL;//=new STRING("U");
		return 'A';
	}
...
namespace v {
	STRING type(ANY &var){return STRING("ANY");}
}
